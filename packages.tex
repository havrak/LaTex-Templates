...
\ifx\PackagesIncludeTikz\IncludeExpr
  ...
  % \usepackage{robust-externalize} <- does the job of externalizing figures but leaves a lot of garbage
...
\usepackage{upgreek} 						% non-cursive greek symbols
\usepackage{mathrsfs} 					% Fancy italic used to denote Hilbert spaces and such
\usepackage{fixmath}            % makes math comply with ISO 31-0:1992 to ISO 31-13:1992 by adding uppercase non-cursive greek symbols
...
\ifx\PackagesIncludeBib\IncludeExpr
  \usepackage[backend=biber,style=iso-numeric,sortlocale=cs_CZ]{biblatex}
...
%%% Minted
\ifx\PackagesIncludeMinted\IncludeExpr
  \usepackage{minted}
